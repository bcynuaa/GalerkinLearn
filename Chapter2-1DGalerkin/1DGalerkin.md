...
首先将区间划分为$n$份，步长$h=\frac{b-a}{n}$，接下来将划分好的区间的$n+1$个节点上标上编号，依次记作$x_0,x_1,x_2\dots,x_n$，有$x_i=x_0+ih$，并将其对应的函数值$y$记作$y_i$。
...
这其实就是一个简单的差分方程，用$\frac{1}{3}\left[ f(x_0) + 2f(x_{\frac{1}{2}}) \right]$取代了$u'(x_0)$项进行的计算。
